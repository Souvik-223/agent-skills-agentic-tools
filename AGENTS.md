...
> This file is auto-loaded by AGENTS.md-compatible AI coding agents (Codex CLI, Cursor, Windsurf, etc.) on every session. Read it fully before responding to any user request.
...
- A test fails after work was declared "done"
- User has to repeat themselves or re-explain
- A mistake is caught mid-task, by the agent itself
- Any rollback or revert of agent changes
...
1. **Immediately acknowledge** — "This mistake has happened before. Here's what should have been done."
...
When a mistake is caught mid-task:
...
3. Were any assumptions made that turned out wrong?
...
**Pattern:** User says "fix the login." Assistant assumes it's a frontend bug. It's actually a backend token expiry issue.
...
**Pattern:** User asks to "add a loading spinner." Assistant also refactors the state management, renames variables, and restructures the component.
...
**Pattern:** Assistant says "I've fixed the bug" but only addressed a symptom. The root cause remains and resurfaces later.
...
**Pattern:** In long conversations, the assistant forgets earlier decisions, constraints, or user preferences and contradicts them.
...
**Pattern:** Assistant states something authoritatively that turns out to be wrong (wrong API, deprecated method, non-existent flag).
...
**Pattern:** Assistant implements 90% of a feature but misses error handling, edge cases, or leaves TODOs.
...
**Pattern:** Assistant edits a file that looks right but is actually the wrong one (e.g., editing a test fixture instead of the source, or a different component with a similar name).
...
> A PostToolUse hook handles this automatically after `git commit` and `git merge` in supporting tools.
